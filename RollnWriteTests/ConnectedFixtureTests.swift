//
//  ConnectedFixtureTests.swift
//  RollnWriteTests
//
//  Replays every golden fixture under spec/fixtures/qwixx-connected/ against
//  the real ConnectedGame engine. Fixture format is normative in
//  spec/fixtures/qwixx-connected/README.md (which extends the base
//  spec/README.md); both this runner and the Android one
//  (android/engine/.../connected/ConnectedFixtureRunnerTest.kt) must
...
// MARK: - Fixture format (Codable mirror of spec/fixtures/qwixx-connected/README.md)
...
/// Every key is optional — assert any subset, per spec/fixtures/qwixx-connected/README.md.
/// Connected introduces no new assert keys over the base spec/README.md vocabulary.
...
@MainActor
final class ConnectedFixtureTests: XCTestCase {
    ...
    private static var fixturesDir: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // ConnectedFixtureTests.swift
            .deletingLastPathComponent() // RollnWriteTests/
            .appendingPathComponent("spec/fixtures/qwixx-connected")
    }
    ...
    private func runFixture(at url: URL) throws {
        ...
        XCTAssertEqual(fixture.variant, "connected", "\(label): unexpected 'variant' \(fixture.variant)")
        XCTAssertEqual(
            fixture.name,
        ...
        // ConnectedGame.save() writes this key to UserDefaults on every
        // mutation; remove it afterwards so test runs don't accumulate
        // orphaned keys.
        let persistenceKey = "test.fixtures.connected.\(UUID().uuidString)"
        addTeardownBlock {
            UserDefaults.standard.removeObject(forKey: persistenceKey)
        }

        // Constructed exactly as QwixxConnectedGame's GameDefinition builds it
        // (see QwixxConnectedGame.swift / ConnectedScorecardView.swift):
        // classic (cap 12) scoring, no bonus rows.
        let game = ConnectedGame(
    ...
    private func assertUnchanged(_ before: StateSnapshot, game: ConnectedGame, context: String) {
    ...
    private func runAssertStep(game: ConnectedGame, file: String, stepIndex: Int, assertion: Assertion) {
