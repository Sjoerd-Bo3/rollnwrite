//
//  DoubleFixtureTests.swift
//  RollnWriteTests
//
//  Replays every golden fixture under spec/fixtures/qwixx-double against the
//  real DoubleGame engine. Fixture format is normative in
//  spec/fixtures/qwixx-double/README.md; both this runner and the Android one
//  (android/engine/.../qwixxdouble/DoubleFixtureRunnerTest.kt) must implement
//  it identically.
//
...
// MARK: - Fixture format (Codable mirror of spec/fixtures/qwixx-double/README.md)
...
private enum Step: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        // A step must be exactly one of "do" / "assert" (fixture README);
...
private struct StateSnapshot: Equatable {
    ...
    let canUndo: Bool
    let canRedo: Bool
    let doubles: [String: Set<Int>]

    @MainActor
    init(_ game: DoubleGame) {
        ...
        doubles = Dictionary(uniqueKeysWithValues: GameColor.allCases.map { ($0.rawValue, game.row(for: $0).doubles) })
    }
}
...
@MainActor
final class DoubleFixtureTests: XCTestCase {
    ...
    private static var fixturesRoot: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // DoubleFixtureTests.swift
            .deletingLastPathComponent() // RollnWriteTests/
            .appendingPathComponent("spec/fixtures/qwixx-double")
    }
    ...
    private func runFixture(at url: URL) throws {
        ...
        XCTAssertEqual(fixture.game, "qwixx-double", "\(label): unexpected 'game' \(fixture.game)")
        XCTAssertEqual(
            fixture.name,
            url.deletingPathExtension().lastPathComponent,
            "\(label): fixture 'name' must match the filename (spec/fixtures/qwixx-double/README.md)"
        )
        ...
        let persistenceKey = "test.fixtures.double.\(UUID().uuidString)"
        addTeardownBlock {
            UserDefaults.standard.removeObject(forKey: persistenceKey)
        }

        // Constructed exactly as QwixxDoubleGame's makeScorecardView() builds
        // its DoubleGame — a TriangularScoring(cap:) sized to the fixture's
        // config, per the fixture README's runner semantics.
        let game = DoubleGame(
    ...
    private func runDoStep(
        ...
        case "doubleColor":
            ...
            let canDo = game.canDoubleColor(gameColor, idx)
            XCTAssertEqual(canDo, expect, "\(context): canDoubleColor(\(colorName), \(idx)) was \(canDo), expected \(expect)")
            let before = StateSnapshot(game)
            game.doubleColor(gameColor, idx)
            if !expect {
                assertUnchanged(before, game: game, context: context)
            }
    ...
    private func assertUnchanged(_ before: StateSnapshot, game: DoubleGame, context: String) {
    ...
    private func runAssertStep(game: DoubleGame, file: String, stepIndex: Int, assertion: Assertion) {
