//
//  BonusFixtureTests.swift
//  RollnWriteTests
//
//  Replays every golden fixture under spec/fixtures/qwixx-bonus/ against the
//  real QwixxBonus (version A) engine, `BonusGame`. Fixture format is
//  normative in spec/README.md ("Qwixx Bonus (version A) fixture family");
//  this runner and the Android one (android/engine) must implement it
//  identically.
//
...
private struct Fixture: Decodable {
    ...
    /// The "bonus" variant carries NO `hasBonusRows` key (there are no
    /// two-colour bonus rows in this variant, only the bar) — unlike
    /// qwixx-big-points/qwixx-classic's `Config`.
...
/// Every key is optional — assert any subset, per spec/README.md. The
/// `bar*` keys are specific to this variant's fixture family (the bonus bar
/// is derived, never marked directly — there is no `markBonus` step here).
private struct Assertion: Decodable {
    ...
    let canRedo: Bool?
    let barEarned: [Int]?
    let barForfeited: [Int]?
    let barEarnedCount: Int?
}
...
private struct StateSnapshot: Equatable {
    ...
    let canUndo: Bool
    let canRedo: Bool
    let barEarned: Set<Int>
    let barForfeited: Set<Int>

    @MainActor
    init(_ game: BonusGame) {
        ...
        barEarned = game.bar.earned
        barForfeited = game.bar.forfeited
    }
}
...
@MainActor
final class BonusFixtureTests: XCTestCase {
    ...
    private static var fixturesRoot: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // BonusFixtureTests.swift
            .deletingLastPathComponent() // RollnWriteTests/
            .appendingPathComponent("spec/fixtures/qwixx-bonus")
    }
    ...
    private func runFixture(at url: URL) throws {
        ...
        XCTAssertEqual(fixture.variant, "bonus", "\(label): unexpected 'variant' \(fixture.variant)")
        XCTAssertEqual(
            fixture.name,
        ...
        let persistenceKey = "test.fixtures.bonus.\(UUID().uuidString)"
        addTeardownBlock {
            UserDefaults.standard.removeObject(forKey: persistenceKey)
        }

        // Constructed exactly as `QwixxBonusGame`'s `makeScorecardView()` ->
        // `QwixxBonusScorecardView` -> `BonusGame()` does (classic Qwixx
        // scoring, cap 12), with the fixture's own persistence key swapped
        // in so tests never touch the real app's saved game.
        let game = BonusGame(
    ...
    private func runDoStep(
        ...
        default:
            XCTFail("\(context): unknown 'do' action '\(action)' (this variant has no 'markBonus' — its bar is derived, never marked directly)")
        }
    }

    private func assertUnchanged(_ before: StateSnapshot, game: BonusGame, context: String) {
    ...
    private func runAssertStep(game: BonusGame, file: String, stepIndex: Int, assertion: Assertion) {
        ...
        if let expected = assertion.barEarned {
            let actual = game.bar.earned
            XCTAssertEqual(actual, Set(expected), "\(context).barEarned: expected \(Set(expected)), got \(actual)")
        }

        if let expected = assertion.barForfeited {
            let actual = game.bar.forfeited
            XCTAssertEqual(actual, Set(expected), "\(context).barForfeited: expected \(Set(expected)), got \(actual)")
        }

        if let expected = assertion.barEarnedCount {
            let actual = game.bar.earnedCount
            XCTAssertEqual(actual, expected, "\(context).barEarnedCount: expected \(expected), got \(actual)")
        }
    }
}
