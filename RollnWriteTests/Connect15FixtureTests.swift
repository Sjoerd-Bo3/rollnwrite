//
//  Connect15FixtureTests.swift
//  RollnWriteTests
//
//  Replays every golden fixture under spec/fixtures/qwixx-connect15 against the
//  real Connect15Game engine. Fixture format is normative in
//  spec/fixtures/qwixx-connect15/README.md; both this runner and the Android
//  one (android/engine, Connect15FixtureRunnerTest) must implement it
//  identically.
//
...
// MARK: - Fixture format (Codable mirror of spec/fixtures/qwixx-connect15/README.md)
...
private enum Step: Decodable {
    case doStep(action: String, color: String?, index: Int?, field: Int?, expect: Bool)
    case assertStep(Assertion)

    private enum CodingKeys: String, CodingKey {
        case `do`, color, index, field, expect, assert
    }
    ...
    init(from decoder: Decoder) throws {
        ...
        if let action = try c.decodeIfPresent(String.self, forKey: .do) {
            ...
            let field = try c.decodeIfPresent(Int.self, forKey: .field)
            self = .doStep(action: action, color: color, index: index, field: field, expect: expect)
        } else {
...
@MainActor
final class Connect15FixtureTests: XCTestCase {
    ...
    private static var fixturesDir: URL {
        URL(fileURLWithPath: #filePath)
            .deletingLastPathComponent() // Connect15FixtureTests.swift
            .deletingLastPathComponent() // RollnWriteTests/
            .appendingPathComponent("spec/fixtures/qwixx-connect15")
    }
    ...
    private func runFixture(at url: URL) throws {
        ...
        XCTAssertEqual(fixture.game, "qwixx-connect15", "\(label): unexpected 'game' \(fixture.game)")
        XCTAssertEqual(
            fixture.name,
            url.deletingPathExtension().lastPathComponent,
            "\(label): fixture 'name' must match the filename (spec/fixtures/qwixx-connect15/README.md)"
        )
        ...
        let persistenceKey = "test.fixtures.connect15.\(UUID().uuidString)"
        addTeardownBlock {
            UserDefaults.standard.removeObject(forKey: persistenceKey)
        }

        // Constructed exactly as QwixxConnect15Game (the GameDefinition)
        // does: Connect15 scoring (cap 15), default scoring strategy.
        let game = Connect15Game(
        ...
        for (index, step) in fixture.steps.enumerated() {
            switch step {
            case let .doStep(action, color, cellIndex, field, expect):
                runDoStep(
                    game: game,
                    file: label,
                    stepIndex: index,
    ...
    private func runDoStep(
        ...
        case "markConnection":
            guard let colorName = color, let gameColor = GameColor(rawValue: colorName), let f = field else {
                XCTFail("\(context): missing/invalid color or field")
                return
            }
            let canDo = game.canMarkConnection(gameColor, field: f)
            XCTAssertEqual(canDo, expect, "\(context): canMarkConnection(\(colorName), field: \(f)) was \(canDo), expected \(expect)")
            let before = StateSnapshot(game)
            game.markConnection(gameColor, field: f)
            if !expect {
                assertUnchanged(before, game: game, context: context)
            }
    ...
    private func assertUnchanged(_ before: StateSnapshot, game: Connect15Game, context: String) {
    ...
    private func runAssertStep(game: Connect15Game, file: String, stepIndex: Int, assertion: Assertion) {
