...
import dev.bo3.rollnwrite.engine.connected.ChainEnd
import dev.bo3.rollnwrite.engine.connected.ConnectedGame
...
/**
 * App-layer host for [ConnectedGame]: owns persistence (SharedPreferences)
 * and exposes engine state as Compose-observable so the board recomposes on
 * every mutation.
 *
 * Mirrors `RollnWrite/Games/QwixxConnected/ConnectedGame.swift`'s
 * persistence section and `BonusViewModel`'s shape (same layering: the
 * engine stays pure JVM/no-Android; this class is the Android host that
 * wires it to `SharedPreferences`, saved on every mutation).
 *
...
class ConnectedViewModel(
    ...
    private val engine = ConnectedGame(scoring = scoring)
     ...
     * value and recomposes. `neverEqualPolicy()` for the same reason as
     * `BonusViewModel.snapshot` — the redo stack lives outside
     * [ConnectedState] (in-memory in the engine), so a commit can change
     * engine-visible behaviour (e.g. `canRedo`) while leaving a
     * structurally-equal state.
     */
    var snapshot: ConnectedState by mutableStateOf(engine.state, neverEqualPolicy())
        private set
    ...
    fun isChainSpace(color: GameColor, index: Int): Boolean = engine.isChainSpace(color, index)

    fun chainPartner(color: GameColor, index: Int): ChainEnd? = engine.chainPartner(color, index)

    fun isMarked(color: GameColor, index: Int): Boolean { snapshot; return engine.isMarked(color, index) }
    ...
    private fun saveToPrefs() {
        val encoded = json.encodeToString(ConnectedState.serializer(), engine.state)
    ...
    private fun loadFromPrefs() {
        ...
        val restored = runCatching { json.decodeFromString(ConnectedState.serializer(), raw) }
            .getOrNull() ?: return
    ...
    companion object {
        ...
        /** Mirrors the iOS persistence key (`ConnectedGame.swift`'s default). */
        const val DEFAULT_KEY_PLAYER_ONE = "rollnwrite.qwixx.connected.state"
...
/**
 * Builds a [ViewModelProvider.Factory] for a [ConnectedViewModel] bound to a
