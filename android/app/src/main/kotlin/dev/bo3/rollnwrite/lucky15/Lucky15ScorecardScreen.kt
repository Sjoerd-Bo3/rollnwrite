...
/**
 * Hosts one Qwixx Lucky15 board: compact in-board header (back, title,
 * 2-player toggle, rules), NO system app bar, per-screen landscape lock, and
 * the optional mirrored two-player layout. Mirrors
 * `RollnWrite/Games/QwixxLucky15/Lucky15ScorecardView.swift`
 * (`QwixxLucky15ScorecardView`) and the wiring pattern of
 * `dev.bo3.rollnwrite.qwixx.QwixxScorecardScreen`.
 *
 * Lucky15 gets its own screen (rather than reusing `QwixxScorecardScreen`)
 * because its board renders a fifth Lucky 15 track band that the shared
 * Qwixx board doesn't know about — same reason iOS has a separate
 * `Lucky15BoardView`/`QwixxLucky15ScorecardView` instead of reusing
 * `QwixxBoardView`.
 */
@Composable
fun Lucky15ScorecardScreen(onBack: () -> Unit) {
    val context = LocalContext.current
    val configuration = LocalConfiguration.current
    var twoPlayer by rememberSaveable { mutableStateOf(false) }
    var showRules by rememberSaveable { mutableStateOf(false) }
    val title = stringResource(R.string.qwixx_lucky15_title)
    ...
    val (playerOneViewModel, playerTwoViewModel) = lucky15ViewModels()
        ...
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            Box(modifier = Modifier.weight(1f).fillMaxSize()) {
                if (twoPlayer) {
                    ...
                    Lucky15BoardView(playerOneViewModel)
                }
            }
        }
    }
    ...
    if (showRules) {
        Lucky15RulesDialog(onDismiss = { showRules = false })
    }
}
 ...
 * mirrors `QwixxScorecardScreen.TwoPlayerBoards`.
 */
@Composable
private fun TwoPlayerBoards(playerOne: Lucky15ViewModel, playerTwo: Lucky15ViewModel) {
    val configuration = LocalConfiguration.current
    val landscape = configuration.screenWidthDp > configuration.screenHeightDp
    if (landscape) {
        Row(modifier = Modifier.fillMaxSize()) {
            Box(modifier = Modifier.weight(1f).fillMaxSize().rotate(180f)) {
                Lucky15BoardView(playerTwo)
            }
            Box(modifier = Modifier.weight(1f).fillMaxSize()) {
                Lucky15BoardView(playerOne)
            }
        }
    } else {
        Column(modifier = Modifier.fillMaxSize()) {
